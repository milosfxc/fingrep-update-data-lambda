...
IF _close > 0 AND _session IS NOT NULL THEN
    ...
    VALUES (NEW.share_id, _timeframe_3h_start, _open, _high, _low, _close, _volume, _vwap, _session)
...
IF _close > 0 AND _session IS NOT NULL THEN
    ...
    VALUES (NEW.share_id, _timeframe_2h_start, _open, _high, _low, _close, _volume, _vwap, _session)
    ON CONFLICT (share_id, datetime) DO UPDATE
    SET
        open   = EXCLUDED.open,
        high   = EXCLUDED.high,
